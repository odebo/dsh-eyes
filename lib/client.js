...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Arrow-function fields so subscribe/snapshot keep their `this` binding
		// when useSyncExternalStore calls them bare — the same shape as the
		// Vision Toolkit controller.
		function EyesController() {
			...
			this.save = async (value, expectedRevision) => {
			...
			this.runHealth = async (testConnection) => {
		...
		function LoadedSettings(props) {
			var controller = props.controller;
			var state = useSyncExternalStore(controller.subscribe, controller.snapshot, controller.snapshot);
			...
			if (snapshot === undefined || draft === undefined) {
				return h("div", { className: "de-settings" },
					h("div", { className: "de-alert error" }, state.error || "设置不可用"),
					h(Button, { variant: "outline", onClick: function () { void controller.load(); } }, "重试"),
				);
			}

			function update(key, val) {
				setDraft(function (cur) { return cur === undefined ? cur : Object.assign({}, cur, { [key]: val }); });
			}
			function save() {
				controller.save(valueOf(draft), snapshot.settings.revision);
			}
			...
			return h("div", { className: "de-settings" },
				...
				h("div", { className: "de-save-row" },
					h(Button, { variant: "primary", disabled: !snapshot.writable || busy, onClick: save }, state.action === "save" ? "保存中…" : "保存并应用"),
					h(Button, { variant: "outline", disabled: busy, onClick: function () { void controller.load(); } }, "重新加载"),
				),
				/* 连接测试 */
				h("section", { className: "de-panel" },
					h("div", { className: "de-panel-title" },
						...
						h("div", { className: "de-actions" },
							h(Button, { size: "sm", variant: "primary", disabled: busy, onClick: function () { void controller.runHealth(true); } }, state.action === "connection" ? "测试中…" : "测试连接"),
						),
					),
