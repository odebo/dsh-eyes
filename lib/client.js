...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* ── draft helpers (only the vision model is editable) ──────── */
		function visionModelOf(value) {
			var v = value || {};
			var vi = v.vision || {};
			return vi.model || "";
		}
		function valueOf(model, credentialRef) {
			return { vision: { model: model, credential: credentialRef } };
		}
		...
		function SettingsSection(props) {
			...
			var savedModel = visionModelOf(snapshot.settings.value);
			var currentModel = draft === null ? savedModel : draft;
			var credRef = snapshot.credential.ref;
			function update(val) { setDraft(val); }
			function save() {
				controller.save(valueOf(currentModel, credRef), snapshot.settings.revision);
				setDraft(null);
			}

			var busy = state.action !== undefined;
			var credOk = snapshot.credential.configured;
			var dirty = draft !== null && draft !== savedModel;
			...
			return h("div", { className: "de-settings" },
				...
				/* 视觉模型 — the one editable field */
				h("section", { className: "de-panel" },
					h("div", { className: "de-panel-title" },
						h("h3", null, "视觉模型"),
						Badge({ ok: credOk }, credOk ? "密钥已配置" : "密钥缺失"),
					),
					h("div", { className: "de-form-grid de-form-one" },
						Field({ label: "多模态模型", hint: "用于理解图片的视觉模型(贴图时自动调用)" },
							h("input", { value: currentModel, onChange: function (e) { update(e.target.value); } }),
						),
					),
					h("p", { className: "de-muted" },
						"API 密钥不在本页配置——请在「设置 → 模型」里配置凭据 ",
						h("code", null, credRef),
						"。" + (credOk ? " 当前:已配置。" : " 当前:未配置。"),
					),
				),
				/* save / reload */
				...
				h("section", { className: "de-panel" },
					h("div", { className: "de-panel-title" },
						h("div", null, h("h3", null, "连接测试"), h("p", null, "把密钥发送到视觉端点验证可用,不上传图片。")),
