 ...
 * Built with schemastery so cordis's standard-schema `resolveConfig` applies
 * defaults and validates at load. Every field defaults to the verified-working
 * mify values, so the plugin works with zero user config.
 */
export const Config = z.object({
  upstream: z.object({
    provider: z.string().default('mify-deepseek'),
    model: z.string().default('tongyi/deepseek-v4-pro'),
  }),
  vision: z.object({
    ...
    prompt: z.string().default('请用简体中文详细描述这张图片的内容。'),
  }),
