...
/**
 * The bridge config. The only thing this tool configures: which multimodal
 * model describes pasted images. The text model is whatever the user selected
 * in the session (options.model), delegated to the fixed mify-deepseek route.
 * The API key is NOT configured here — it lives in 设置 → 模型 (the dsh
 * credentials service); `credential` is just the reference name of that key.
 * Defaults are the verified-working mify values.
 */
