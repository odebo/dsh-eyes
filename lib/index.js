...
/**
 * The bridge config — the "视觉服务" surface, mirroring the Vision Toolkit
 * card: which multimodal model describes pasted images (model), where its
 * Anthropic-compatible endpoint is (baseURL), and which credential holds the
 * API key (credential — the key itself is configured in 设置 → 模型, this is
 * just the reference name). The text model is NOT configured: it is whatever
 * the user selected in the session, delegated to the fixed mify-deepseek
 * route. Defaults are the verified-working mify values.
 */
...
function createAdapter(ctx, config) {
  ...
  return {
    ...
    async listModels(_provider) {
      // The bridge wraps the real text models: list them so the model picker
      // shows the user which text model they're selecting (with eyes attached).
      // We re-home the upstream mify-deepseek catalog under deepseek-eyes and
      // declare image input on every entry so the picker/admission gate treat
      // them all as image-capable.
      try {
        const upstream = await ctx.llm.listModels(UPSTREAM_PROVIDER)
        return upstream.map((m) => ({
          provider: 'deepseek-eyes',
          id: m.id,
          name: m.name,
        ...
        // If the upstream adapter isn't queryable, fall back to an empty
        // catalog; resolveModel still admits any model by name.
        return []
      }
    },
...
export function apply(ctx, config = {}) {
  ...
  // registerAdapter is what makes deepseek-eyes appear in the model picker
  // (listProviders reads the adapter registry). No registerConfigurableProviders:
  // that would add a second, conflicting config entry in 设置 → 模型 — the
  // vision-service config lives only on this plugin's own Settings card.
