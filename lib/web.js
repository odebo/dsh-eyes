...
export class DshEyesWebBackend {
  /** Whether the deepseek-eyes adapter is currently registered. */
  ...
  /** Local health + optional live connection test against the vision endpoint. */
  async health(request, req) {
    ...
    const checks = {
      ...
      adapter: {
        ...
        detail: this.adapterRegistered ? 'deepseek-eyes adapter registered' : 'adapter not registered',
      },
    }
    let connection
    if (request.testConnection) {
      if (!credential.configured) {
        checks.endpoint = { status: 'error', detail: 'skipped: credential missing' }
        ...
        try {
          ...
          const base = (typeof vision.baseURL === 'string' ? vision.baseURL : 'https://api.llm.mioffice.cn/anthropic').replace(/\/+$/, '')
          // The OpenAI-compatible /v1/models path on the same gateway verifies key + endpoint.
          const res = await fetch(`${base.replace('/anthropic', '/v1')}/models`, {
          ...
          if (res.ok) {
            ...
            checks.endpoint = { status: 'ok', detail: `reachable${count !== undefined ? ` (${count} models)` : ''}` }
            connection = { ok: true, modelCount: count }
          } else {
            checks.endpoint = { status: 'error', detail: `HTTP ${res.status}` }
            connection = { ok: false, detail: `HTTP ${res.status}` }
          }
        } catch (error) {
          checks.endpoint = { status: 'error', detail: publicMessage(error) }
